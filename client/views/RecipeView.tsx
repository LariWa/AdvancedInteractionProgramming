...
export default function RecipeView(props: any) {
  function renderArrayCB(ingredient: any){
    return<ListItem
      ...
      key={ingredient.name}
      trailing={props =>  <AntDesign name="pluscircleo" size={24} color="black" />}
    />
  }
  return <Flex fill direction="column">
      <Image
        style={styles.image}
        source={props.recipe.strMealThumb}
      ></Image>
      <Flex
        direction="column"
        style={styles.details}>
          ...
          <>
            <View>{props.recipe.ingredients && props.recipe.ingredients.map(renderArrayCB)} </View>
            <ListItem
              title="Ingr1"
              trailing={props =>  <AntDesign name="pluscircleo" size={24} color="black" />}
            />
